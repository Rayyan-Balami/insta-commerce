...
export default function DiscountTable({ setIsEdit, setEditDataID }) {
  ...
  const initialData = useSelector((state) => state.promotion.promotions.discounts);
  ...
  const table = useReactTable({
    data,
    columns: createColumns(handleCheckboxChange, setIsEdit, setEditDataID),
  ...
  return (
    <>
      <TableControls table={table} searchColumns={[{ label: "Name", column: "name" }]} filters={filters}>
        <AlertDialog
          title={<><span>Remove Discounts</span> <Badge variant="outline">{Object.keys(rowSelection).length}</Badge></>}

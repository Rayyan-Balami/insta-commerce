...
const Carousel = forwardRef(
  (
    ...
    useEffect(() => {
      ...
      const startAutoplay = () => {
        emblaMainApi.reInit();
        onSelect();  // Ensure the select event is set correctly
        if (emblaMainApi.plugins && emblaMainApi.plugins.length > 0) {
          emblaMainApi.plugins.forEach(plugin => {
  ...
  ({ className, index, children, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(
          "flex shrink-0 grow-0 bg-background p-1 cursor-pointer",
          `${orientation === "vertical" ? "pb-1" : "pr-1"}`,
          className
        )}
      >
        <div
          className={`relative size-36 aspect-square border overflow-hidden w-full opacity-40 rounded-md transition-opacity ${
